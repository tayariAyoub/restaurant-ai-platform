...
export default function AdminShell({ children }: { children: React.ReactNode }) {
  ...
  if (!user) {
    return (
      <main className="grid min-h-screen place-items-center bg-[#f4f1ea] text-slate-500">
        <div className="premium-card rounded-2xl px-6 py-5">Opening your workspace...</div>
      </main>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-[#f4f1ea] text-slate-900">
      <aside className="fixed inset-y-0 left-0 z-30 hidden w-72 border-r border-white/10 bg-[#12120f] p-5 text-white shadow-2xl lg:block">
        ...
        <p className="mt-2 text-xs text-slate-400">The operating system for independent restaurants</p>
        ...
        <div className="absolute bottom-5 left-5 right-5 rounded-2xl border border-white/10 bg-white/5 p-3 backdrop-blur">
      ...
      <div className="lg:pl-72">
        <header className="sticky top-0 z-20 flex h-16 items-center justify-between border-b border-black/5 bg-white/80 px-5 backdrop-blur-xl lg:px-8">
          ...
          <Link
            ...
            target="_blank"
            className="flex items-center gap-2 rounded-xl border bg-white px-3 py-2 text-sm font-medium shadow-sm transition hover:-translate-y-0.5 hover:bg-slate-50"
          >
