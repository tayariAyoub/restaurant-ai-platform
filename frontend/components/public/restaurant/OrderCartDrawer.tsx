    ...
    <div className="fixed inset-0 z-[60] flex items-end justify-center bg-black/60 p-0 backdrop-blur-sm sm:items-center sm:p-5">
      <div className="max-h-[calc(100svh-0.75rem)] w-full max-w-2xl overflow-y-auto rounded-t-[2rem] bg-[#fbfaf7] p-4 pb-[calc(1rem+env(safe-area-inset-bottom))] text-slate-900 shadow-2xl sm:max-h-[94vh] sm:rounded-[2rem] sm:p-6">
        <div className="flex items-center justify-between">
          <div>
            <p className="luxury-kicker text-xs font-bold" style={{ color: primary }}>Online bestellen</p>
            <h2 className="text-3xl font-semibold">Ausgewählte Gerichte</h2>
          </div>
          <button onClick={() => { setCartOpen(false); setCompletedOrder(null); }} className="grid h-11 w-11 place-items-center rounded-full border bg-white" aria-label="Warenkorb schließen"><X /></button>
        </div>
        ...
        {completedOrder ? (
          <div className="mt-8 overflow-hidden rounded-3xl border border-green-100 bg-green-50">
            <div className="p-6 text-center sm:p-8">
              ...
              <p className="mt-5 text-xs font-bold uppercase tracking-[0.22em] text-green-700">Bestellung bestätigt</p>
              <h3 className="mt-2 text-3xl font-bold">#{shortOrderNumber(completedOrder.public_id)}</h3>
              <p className="mt-2 text-sm leading-6 text-green-900">
                {restaurant.name} hat Ihre Bestellung erhalten. Halten Sie diese Nummer bereit, falls Sie das Restaurant anrufen.
              </p>
            </div>

            <div className="grid gap-3 border-y border-green-100 bg-white/80 p-4 text-sm sm:grid-cols-3">
              <SuccessMetric label="Gesamt" value={formatPrice(completedOrder.total)} />
              <SuccessMetric label="Art" value={orderTypeLabel(completedOrder.order_type)} />
              <SuccessMetric label="Zeitfenster" value={completedEstimate} />
            </div>
            ...
            <div className="space-y-4 p-5 sm:p-6">
              ...
              <div className="rounded-2xl bg-white p-4 text-sm leading-6 text-slate-600">
                <p className="font-semibold text-slate-900">Wie geht es weiter?</p>
                <p className="mt-1">{nextInstruction(completedOrder.order_type)}</p>
                {restaurant.phone && (
                  <p className="mt-3">
                    Hilfe benötigt? Rufen Sie {restaurant.name} unter <a className="font-semibold underline" href={`tel:${restaurant.phone}`}>{restaurant.phone}</a> an.
                  </p>
                )}
              </div>
          ...
          <form onSubmit={submitOrder}>
            <div className="mt-6 grid grid-cols-3 gap-2 text-center text-xs font-bold text-slate-500">
              {["Prüfen", "Details", "Bestätigen"].map((step, index) => (
              ...
              {cartLines.map((line) => (
                <div key={line.item.id} className="grid gap-3 rounded-2xl border border-black/5 bg-white p-3 shadow-sm sm:flex sm:items-center sm:justify-between">
                  <div className="min-w-0">
                    ...
                    <p className="text-sm text-slate-500">{formatPrice(line.item.price)} pro Stück</p>
                  </div>
                  <div className="flex items-center justify-between gap-2 sm:justify-end">
                    <button type="button" onClick={() => changeCart(line.item, -1)} className="grid h-11 w-11 place-items-center rounded-xl border bg-white" aria-label={`${line.item.name} einmal entfernen`}><Minus size={15} /></button>
                    <b className="min-w-8 text-center">{line.quantity}</b>
                    <button type="button" onClick={() => changeCart(line.item, 1)} className="grid h-11 w-11 place-items-center rounded-xl border bg-white" aria-label={`${line.item.name} einmal hinzufügen`}><Plus size={15} /></button>
                    <button type="button" onClick={() => removeItem(line.item.id)} className="ml-auto grid h-11 w-11 place-items-center rounded-xl text-red-600 sm:ml-2" aria-label={`${line.item.name} entfernen`}><Trash2 size={17} /></button>
                  </div>
                </div>
              ))}
            </div>
            ...
            <div className="mt-6">
              <p className="text-sm font-bold">Wie möchten Sie bestellen?</p>
            ...
            <div className="mt-6 grid gap-3 sm:grid-cols-2">
              ...
              {orderType === "DELIVERY" && (
                <>
                  ...
                  <input required name="delivery_city" autoComplete="address-level2" placeholder="Stadt" className="min-h-12 rounded-xl border px-4 py-3 text-base sm:text-sm" />
                  <input name="delivery_instructions" placeholder="Klingel, Etage, Lieferhinweise" className="min-h-12 rounded-xl border px-4 py-3 text-base sm:col-span-2 sm:text-sm" />
                </>
              )}
              <textarea name="notes" placeholder="Hinweise für das Restaurant" className="min-h-24 rounded-xl border px-4 py-3 text-base sm:col-span-2 sm:text-sm" />
            </div>
            ...
            <button disabled={cartLines.length === 0 || orderSubmitting} className={`luxury-button mt-5 flex w-full items-center justify-center gap-2 ${buttonClass} py-4 font-bold text-white disabled:opacity-50`} style={{ backgroundColor: primary }}>
              ...
              {orderSubmitting ? "Bestellung wird bestätigt..." : "Bestellung bestätigen"}
            </button>
          </form>
        )}
      </div>
...
function orderModeLabel(value: RestaurantOrder["order_type"]) {
  if (value === "EAT_IN") return "Vor Ort essen";
...
function OrderTimeline({ status, orderType }: { status: string; orderType: RestaurantOrder["order_type"] }) {
  ...
  return (
    <div className="rounded-2xl bg-white p-4">
      <p className="font-semibold text-slate-900">Bestellstatus</p>
