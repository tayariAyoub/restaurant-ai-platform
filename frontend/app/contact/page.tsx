...
export default function ContactPage() {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setStatus("Thank you. We will contact you shortly to confirm.");
    } catch (error) {
                  ...
                  <div>
                    ...
                    <div className="mt-2 rounded-2xl bg-white p-4 shadow-sm">
                      {Object.entries(hours).map(([day, value]) => (
                        <p key={day} className="flex justify-between gap-4 border-b border-black/5 py-2 text-sm last:border-0">
                          <span className="capitalize">{day}</span><span className="font-semibold">{value}</span>
                        </p>
                      ))}
                    </div>
                  </div>
